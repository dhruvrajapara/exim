...
export default function BlogPageHeroSection() {
  ...
  const fetchData = async () => {
    try {
      const settingsRes = await fetch('/api/section-settings/blog_page_hero');
      if (settingsRes.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('/api/admin/section-settings/blog_page_hero', {
        method: 'POST',
        headers: {
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" sx={{ mb: 3 }}>Blog Page Hero Settings</Typography>
      
      <Paper sx={{ p: 3, maxWidth: 800 }}>
        <Typography variant="body1" sx={{ mb: 3, color: 'text.secondary' }}>
          Update the dynamic text, colors, and background image for the Hero Banner on the Blog Page.
        </Typography>

        <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          <TextField 
            label="Title (e.g. Export Blog & Industry Insights)" 
            name="title" 
