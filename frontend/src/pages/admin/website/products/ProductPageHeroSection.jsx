...
export default function ProductPageHeroSection() {
  ...
  const fetchData = async () => {
    try {
      const settingsRes = await fetch('/api/section-settings/product_page_hero');
      if (settingsRes.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('/api/admin/section-settings/product_page_hero', {
        method: 'POST',
        headers: {
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" sx={{ mb: 3 }}>Product Page Hero Settings</Typography>
      
      <Paper sx={{ p: 3, maxWidth: 800 }}>
        <Typography variant="body1" sx={{ mb: 3, color: 'text.secondary' }}>
          Update the dynamic text, colors, and background image for the Hero Banner on the Products Page.
        </Typography>
