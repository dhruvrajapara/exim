...
export default function GlobalGallery() {
    ...
    const fetchImages = async () => {
      try {
        ...
        if (data.status === 'success') {
          ...
          console.error('Error fetching gallery images', data);
        }
      } catch (error) {
  ...
  if (isLoading) {
    return (
      <section className="py-12 md:py-16 bg-white min-h-[400px] flex justify-center items-center">
    ...
    return (
      <section className="py-12 md:py-16 bg-white">
        <div className="container-custom text-center py-20">
          <h2 className="text-2xl font-bold text-gray-400">No images available in the gallery yet.</h2>
        </div>
      </section>
    );
  }
